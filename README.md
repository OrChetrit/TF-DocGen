...
**Terraform Module Documentation Generator** — a professional-grade CLI tool for Cloud
Governance teams that automatically produces standardised `README.md` files from Terraform
module source code.
...
Large organisations running infrastructure at scale face a well-known documentation
problem: engineers write Terraform modules that work correctly but are poorly described,
making them hard to discover, reuse, or audit.

TF-DocGen solves this by treating documentation as a first-class artefact of the
...
- **python-hcl2** is used for all HCL parsing — no regex, so the tool handles complex
  type constraints, heredocs, and multi-file modules without brittle string manipulation.
- **Dataclasses** (`ModuleData`, `TerraformVariable`, `TerraformOutput`, `TerraformResource`)
  decouple the parse stage from the render stage, making it easy to add new output
  formats (JSON schema, Confluence pages, etc.) without touching the parser.
- **Jinja2** keeps all Markdown structure in one template file. Teams can fork
  `template.j2` to enforce a company-specific layout without changing any Python code.
- **Governance logic** lives in `TerraformVariable.governance_issues` — a pure property
  that can be unit-tested independently of the CLI.
...
Violations are printed to `stderr` with a `[FAIL]` prefix. In `--strict` mode the
process exits with code `1`, blocking merges in pull-request pipelines.
...
The `ModuleData` dataclass is format-agnostic. Introduce a new renderer in
`main.py` alongside `render_readme()`, consuming the same `ModuleData` object.
